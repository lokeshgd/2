...
function Install-Node {
    ...
    try {
        ...
        if (Test-CommandExists 'winget') {
            ...
            Write-Log 'Node.js is required but winget is unavailable and no fallback URL provided.' -Level Warn
        }
    }
    catch {
...
function Install-Antigravity {
    ...
    try {
        $binary = Expand-ConfigPath $ToolConfig.binary
        if (Test-Path $binary) {
            Write-Log "Antigravity CLI already installed at $binary"
            return
        }

        Write-Log 'Installing Antigravity CLI...'
        $scriptPath = Join-Path $env:TEMP 'antigravity-install.ps1'
        Invoke-WebRequest -Uri $ToolConfig.installScriptUrl -OutFile $scriptPath -UseBasicParsing
        & $scriptPath
        Update-Path

        if (Test-Path $binary -or (Test-CommandExists 'agy')) {
            Write-Log "Antigravity CLI installed successfully."
        }
        else {
            Write-Log 'Antigravity CLI install finished but agy was not found.' -Level Warn
        }
    }
    catch {
